...
/*   Updated: 2021/03/19 21:38:37 by fcadet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_min_on_top(t_glob *glob, t_2_stacks *stacks, t_stack **result)
{
	...
	pos = TOP;
	if (stacks->a->length > 1 && (tmp = *((int **)stacks->a->data)[0]) < min_val)
	{
		pos = BOTTOM;
		min_val = tmp;
	}
...
t_stack		*insertion_sort(t_glob *glob, t_2_stacks *stacks)
{
	...
	while (!sorted(stacks))
	{
		...
		if (sorted(stacks))
			break ;
		if (!stacks->b->length || min > *((int **)stacks->b->data)[stacks->b->length - 1])
